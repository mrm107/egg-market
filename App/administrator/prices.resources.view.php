...
$controller->view->view_title = 'منابع';
$controller->set_sidebar();
$controller->checker_page_access(115);
$errors = '';
//users start
if (isset($_POST['delet-selected'])) {
    foreach ($_REQUEST as $key => $val) {
        if (strpos($key, 'delete') !== false) {
            ...
            $access = $controller->model->db->prepare("delete from prices_resources WHERE `id` = :delete");
...
<a class='plusmenu' href='prices.resources.new.php'>منبع جدید</a>
...
if ((isset($_POST['search']) and trim($_POST['search']) != '') or (isset($_GET['search']) and trim($_GET['search']) != '')) {
    if (isset($_POST['search']))
        ...
        $search = $_GET['search'];
    $realsearch = $search;
    $user = $controller->model->db->prepare("select * from prices_resources where (`title` LIKE CONCAT('%', :search, '%')) order by `sort` ASC, `title` ASC {$limit}");
    $user->execute(array(':search' => $search));
    $status = '&search=' . $search;
} else {
    $realsearch = '';
    $user = $controller->model->db->prepare("select * from prices_resources order by `sort` ASC, `title` ASC {$limit}");
    $user->execute();
    $status = '';
}
$realstatus = '';
...
$controller->view_box_header_beta .= "
	<span title='قیمت گذاری -> منابع'>منابع</span>
	<div class='ui buttons mini custome-tools-button-place'>
	  <input type='submit' name='delet-selected' form='content' formaction='prices.resources.view.php?{$status}' class='ui button custome-tools-button' value='ح' title='حذف' onclick='return delete_checker()' />
	  <div class='or'></div>
	  <input type='submit' name='edit-selected' form='content' formaction='prices.resources.edit.php?{$status}' class='ui button custome-tools-button' value='و' title='ویرایش' />
	</div>
	{$errors}
...
foreach ($users as $key) {
    ...
    $controller->view_content_beta .= "<td>{$key['description']}</td>";
    $controller->view_content_beta .= "<td>{$key['sort']}</td>";
...
if ((isset($_POST['search']) and trim($_POST['search']) != '') or (isset($_GET['search']) and trim($_GET['search']) != '')) {
    if (isset($_POST['search']))
        ...
        $search = $_GET['search'];
    $realsearch = $search;
    $user = $controller->model->db->prepare("SELECT * FROM prices_resources WHERE (`title` LIKE CONCAT('%', :search, '%')) ORDER BY `sort` ASC, `title` ASC ");
    $user->execute(array(':search' => $search));
    $status = '&search=' . $search;
} else {
    $realsearch = '';
    $user = $controller->model->db->prepare("SELECT * FROM prices_resources ORDER BY `sort` ASC, `title` ASC ");
    $user->execute();
    $status = '';
}
