...
$controller->view->view_title = 'ویرایش آگهی';
$controller->checker_page_access(98);
$errors = '';
if (!isset($_REQUEST['edit']) or (isset($_REQUEST['edit']) and intval($_REQUEST['edit']) == '')) {
    header('Location: loads.view.php');
    exit;
} else {
    $editid = intval($_REQUEST['edit']);
    $editdoc = $controller->model->db->prepare("SELECT * FROM `loads` WHERE id = :id");
...
if (isset($_POST['submit'])) {
    try {
        ...
        $statement = $controller->model->db->prepare("UPDATE `loads` SET `origin_field1` = :origin_field1, `origin_field2` = :origin_field2, `status` = :status, `description` = :description, `owner_name` = :owner_name, `weight` = :weight, `count` = :count, `print_type` = :print_type, `yolk_type` = :yolk_type, `box_type` = :box_type, `stage_type` = :stage_type, `person_owner_name` = :person_owner_name, `type` = :type, `pack_type` = :pack_type, `reg_customer` = :reg_customer, `price` = :price, `quality` = :quality WHERE `id` = :id ");
        ...
        $delete_phones = $controller->model->db->prepare("delete from `loads_phones` where `load` = :id");
        $delete_phones->execute(array(":id" => $editid));
            ...
            if (strpos($key, 'phone-') !== false) {
                if ($controller->check_null_no($value) != NULL) {
                    ...
                    if (!in_array($phone, $phone_arrays)) {
                        ...
                        $add_phone->execute(array(":load" => $editid, ":phone" => utils__fatoen_numbers($phone)));
                        $phone_arrays[] = $phone;
                    }
                }
            }
...
$controller->view->view_box_header = "<span title='آگهی ها -> ویرایش آگهی'>ویرایش آگهی {$errors}</span>";
		...
		<div class='form-elements'>
            <div>
                ...
                <select name='type' required>
                    <option " . ($editdoc['type'] == 'announcement' ? 'selected' : '') . " value='announcement' selected>اعلام بار</option>
                    <option " . ($editdoc['type'] == 'request' ? 'selected' : '') . " value='request'>درخواست بار</option>
                </select>
            </div>
            <div>
                <h6>نوع بسته بندی</h6>
                <select name='pack_type' required>
                    <option></option>
                    <option " . ($editdoc['pack_type'] == 'bulk' ? 'selected' : '') . " value='bulk'>فله (شانه ای)</option>
                    <option " . ($editdoc['pack_type'] == 'box' ? 'selected' : '') . " value='box'>بسته بندی</option>
                </select>
            </div>
          <div>
                <h6>وزن/تعداد در کارتن</h6>
                <input type='text' name='weight' value='{$editdoc['weight']}' required>
            </div>
            <div>
                <h6>تعداد کارتن</h6>
                <input type='text' name='count' value='{$editdoc['count']}' required>
            </div>
            <div>
                <h6>نوع زرده</h6>
                <select name='yolk_type'>
                    <option></option>
                    <option value='golden' " . ($editdoc['yolk_type'] == 'golden' ? 'selected' : '') . ">طلایی</option>
                    <option value='simple' " . ($editdoc['yolk_type'] == 'simple' ? 'selected' : '') . ">ساده</option>
                    <option value='corn' " . ($editdoc['yolk_type'] == 'corn' ? 'selected' : '') . ">ذرتی</option>
                </select>
            </div>
            <div>
                <h6>نوع پرینت</h6>
                <select name='print_type'>
                    <option></option>
                    <option value='with' " . ($editdoc['print_type'] == 'with' ? 'selected' : '') . ">با پرینت</option>
                    <option value='without' " . ($editdoc['print_type'] == 'without' ? 'selected' : '') . ">بدون پرینت</option>
                    <option value='ability' " . ($editdoc['print_type'] == 'ability' ? 'selected' : '') . ">با قابلیت پرینت</option>
                </select>
            </div>
            <div>
                <h6>نوع کارتن</h6>
                <input type='text' name='box_type' value='{$editdoc['box_type']}'>
            </div>            
            <div>
                <h6>نوع شانه</h6>
                <input type='text' name='stage_type' value='{$editdoc['stage_type']}'>
            </div>
            <div>
                <h6>کیفیت</h6>
                <select name='quality'>
                    <option></option>
                    <option value='lux' " . ($editdoc['quality'] == 'lux' ? 'selected' : '') . ">لوکس</option>
                    <option value='grade-1' " . ($editdoc['quality'] == 'grade-1' ? 'selected' : '') . ">درجه ۱</option>
                    <option value='grade-2' " . ($editdoc['quality'] == 'grade-2' ? 'selected' : '') . ">درجه ۲</option>
                    <option value='for-factories' " . ($editdoc['quality'] == 'for-factories' ? 'selected' : '') . ">درجه ۳ (کارخانه‌ای)</option>
                </select>
            </div>
            <div>
                <h6>قیمت</h6>
                <input type='text' name='price' value='{$editdoc['price']}'>
            </div>
...
foreach ($provinces as $province) {
    if ($editdoc['origin_field1'] == $province['id']) {
...
$controller->view_content_beta .= "
            ...
            <div>
                <h6>محل: منطقه</h6>
                <input type='text' name='origin_field2' value='{$editdoc['origin_field2']}' id='load-origin-field2' placeholder='شهرک صنعتی فلان ...' maxlength='200'>
            </div>
            <div>
                <h6>وضعیت</h6>
                <select name='status' required>
                    <option value='accepted' " . ($editdoc['status'] == 'accepted' ? 'selected' : '') . ">تایید شده</option>
                    <option value='rejected' " . ($editdoc['status'] == 'rejected' ? 'selected' : '') . ">رد شده</option>
                    <option value='pending' " . ($editdoc['status'] == 'pending' ? 'selected' : '') . ">در انتظار</option>
                    <option value='deactive' " . ($editdoc['status'] == 'deactive' ? 'selected' : '') . ">حذف شده</option>
                    <option value='sold' " . ($editdoc['status'] == 'sold' ? 'selected' : '') . ">فروخته شده</option>
                    <option value='expired' " . ($editdoc['status'] == 'expired' ? 'selected' : '') . ">منقضی شده</option>
                </select>
            </div>
            <div>
                <h6>توضیحات</h6>
                <textarea name='description'>{$editdoc['description']}</textarea>
            </div>
            <div>
                <h6>جستجو</h6>
            ...
            <div>
                <h6>مشتری متصل</h6>
                <input name='reg_customer' type='hidden' value='{$editdoc['reg_customer']}' />
...
foreach ($customers as $customer) {
    if ($editdoc['reg_customer'] == $customer['id']) {
        $controller->view_content_beta .= "<option value='{$customer['id']}' selected>{$customer['name']}</option>";
    } else {
...
$loads_phones = $controller->model->db->prepare("SELECT * FROM loads_phones WHERE `load` = :load order by `id` ASC");
$loads_phones->execute(array(":load" => $editid));
$loads_phones = $loads_phones->fetchAll();
$i = 0;
foreach ($loads_phones as $item) {
    $i++;
    $req = $i == 1 ? 'required' : '';
    $controller->view_content_beta .= "<div><h6>شماره تماس {$i}</h6><input type='text' value='{$item['phone']}' name='phone-{$i}' {$req}></div>";
}
while ($i < 4) {
    $i++;
    $req = $i == 1 ? 'required' : '';
