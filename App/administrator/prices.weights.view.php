...
$controller->view->view_title = 'وزن ها';
$controller->set_sidebar();
$controller->checker_page_access(113);
$errors = '';
//users start
if (isset($_POST['delet-selected'])) {
    foreach ($_REQUEST as $key => $val) {
        if (strpos($key, 'delete') !== false) {
            ...
            $access = $controller->model->db->prepare("delete from prices_weights WHERE `id` = :delete");
...
<a class='plusmenu' href='prices.weights.new.php'>وزن جدید</a>
...
if ((isset($_POST['search']) and trim($_POST['search']) != '') or (isset($_GET['search']) and trim($_GET['search']) != '')) {
    if (isset($_POST['search']))
        ...
        $search = $_GET['search'];
    $realsearch = $search;
    $user = $controller->model->db->prepare("select * from prices_weights where (`title` LIKE CONCAT('%', :search, '%')) order by `sort` ASC, `title` ASC {$limit}");
    $user->execute(array(':search' => $search));
    $status = '&search=' . $search;
} else {
    $realsearch = '';
    $user = $controller->model->db->prepare("select * from prices_weights order by `sort` ASC, `title` ASC {$limit}");
    $user->execute();
    $status = '';
}
$realstatus = '';
...
$controller->view_box_header_beta .= "
	<span title='قیمت گذاری -> وزن ها'>وزن ها</span>
	<div class='ui buttons mini custome-tools-button-place'>
	  <input type='submit' name='delet-selected' form='content' formaction='prices.weights.view.php?{$status}' class='ui button custome-tools-button' value='ح' title='حذف' onclick='return delete_checker()' />
	  <div class='or'></div>
	  <input type='submit' name='edit-selected' form='content' formaction='prices.weights.edit.php?{$status}' class='ui button custome-tools-button' value='و' title='ویرایش' />
	</div>
	{$errors}
...
if ((isset($_POST['search']) and trim($_POST['search']) != '') or (isset($_GET['search']) and trim($_GET['search']) != '')) {
    if (isset($_POST['search']))
        ...
        $search = $_GET['search'];
    $realsearch = $search;
    $user = $controller->model->db->prepare("SELECT * FROM prices_weights WHERE (`title` LIKE CONCAT('%', :search, '%')) ORDER BY `sort` ASC, `title` ASC ");
    $user->execute(array(':search' => $search));
    $status = '&search=' . $search;
} else {
    $realsearch = '';
    $user = $controller->model->db->prepare("SELECT * FROM prices_weights ORDER BY `sort` ASC, `title` ASC ");
    $user->execute();
    $status = '';
}
