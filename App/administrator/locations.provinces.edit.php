...
$controller->view->view_title = 'ویرایش استان';
$controller->checker_page_access(94);
$errors = '';
if (!isset($_REQUEST['edit']) or (isset($_REQUEST['edit']) and intval($_REQUEST['edit']) == '')) {
    header('Location: locations.provinces.view.php');
    exit;
} else {
    $editid = intval($_REQUEST['edit']);
    $editdoc = $controller->model->db->prepare("SELECT * FROM `locations_provinces` WHERE id = :id");
...
if (isset($_POST['submit'])) {
    try {
        ...
        $statement = $controller->model->db->prepare("UPDATE `locations_provinces` SET `title` = :title WHERE `id` = :id ");
...
$controller->view->view_box_header = "<span title='مکان ها -> ویرایش استان'>ویرایش استان {$errors}</span>";
