...
$controller->view->view_title = 'ویرایش وزن';
$controller->checker_page_access(113);
$errors = '';
if (!isset($_REQUEST['edit']) or (isset($_REQUEST['edit']) and intval($_REQUEST['edit']) == '')) {
    header('Location: prices.weights.view.php');
    exit;
} else {
    $editid = intval($_REQUEST['edit']);
    $editdoc = $controller->model->db->prepare("SELECT * FROM `prices_weights` WHERE id = :id");
...
$controller->view->view_box_header = "<span title='قیمت گذاری -> ویرایش وزن'>ویرایش وزن {$errors}</span>";
