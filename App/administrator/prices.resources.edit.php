...
$controller->view->view_title = 'ویرایش منبع';
$controller->checker_page_access(115);
$errors = '';
if (!isset($_REQUEST['edit']) or (isset($_REQUEST['edit']) and intval($_REQUEST['edit']) == '')) {
    header('Location: prices.resources.view.php');
    exit;
} else {
    $editid = intval($_REQUEST['edit']);
    $editdoc = $controller->model->db->prepare("SELECT * FROM `prices_resources` WHERE id = :id");
...
if (isset($_POST['submit'])) {
    try {
        ...
        $statement = $controller->model->db->prepare("UPDATE `prices_resources` SET `title` = :title, `description` = :description, `sort` = :sort WHERE `id` = :id ");
        $statement->execute(array(
            ":title" => utils__arabic_character_to_persian($_POST['title']),
            ":description" => utils__arabic_character_to_persian($_POST['description']),
...
$controller->view->view_nav = "<a class='plusmenu' href='prices.resources.view.php'>بازگشت</a>" . $controller->set_fastaccess();
$controller->view->view_box_header = "<span title='قیمت گذاری -> ویرایش منبع'>ویرایش منبع {$errors}</span>";
...
$controller->view_content_beta .= "
	<div class='content-place-full' id='scroll-place'>
		...
		<div class='form-elements'>
            ...
            <div>
			    <h6>توضیحات منبع</h6>
			    <input autocomplete='off' type='text' name='description' value='{$editdoc['description']}'>    
            </div>
            <div>
			    <h6>اولویت</h6>
