...
$controller->view->view_title = 'شهر ها';
$controller->set_sidebar();
$controller->checker_page_access(112);
$errors = '';
//users start
if (isset($_POST['delet-selected'])) {
    foreach ($_REQUEST as $key => $val) {
        if (strpos($key, 'delete') !== false) {
            ...
            $access = $controller->model->db->prepare("delete from prices_cities WHERE `id` = :delete");
...
<a class='plusmenu' href='prices.cities.new.php'>شهر جدید</a>
...
if ((isset($_POST['search']) and trim($_POST['search']) != '') or (isset($_GET['search']) and trim($_GET['search']) != '')) {
    if (isset($_POST['search']))
        ...
        $search = $_GET['search'];
    $realsearch = $search;
    $user = $controller->model->db->prepare("select * from prices_cities where (`title` LIKE CONCAT('%', :search, '%')) order by `sort` ASC, `title` ASC {$limit}");
    $user->execute(array(':search' => $search));
    $status = '&search=' . $search;
} else {
    $realsearch = '';
    $user = $controller->model->db->prepare("select * from prices_cities order by `sort` ASC, `title` ASC {$limit}");
    $user->execute();
    $status = '';
}
$realstatus = '';
...
$controller->view_box_header_beta .= "
	<span title='قیمت گذاری -> شهر ها'>شهر ها</span>
	<div class='ui buttons mini custome-tools-button-place'>
	  <input type='submit' name='delet-selected' form='content' formaction='prices.cities.view.php?{$status}' class='ui button custome-tools-button' value='ح' title='حذف' onclick='return delete_checker()' />
	  <div class='or'></div>
	  <input type='submit' name='edit-selected' form='content' formaction='prices.cities.edit.php?{$status}' class='ui button custome-tools-button' value='و' title='ویرایش' />
	</div>
	{$errors}
...
if ((isset($_POST['search']) and trim($_POST['search']) != '') or (isset($_GET['search']) and trim($_GET['search']) != '')) {
    if (isset($_POST['search']))
        ...
        $search = $_GET['search'];
    $realsearch = $search;
    $user = $controller->model->db->prepare("SELECT * FROM prices_cities WHERE (`title` LIKE CONCAT('%', :search, '%')) ORDER BY `sort` ASC, `title` ASC ");
    $user->execute(array(':search' => $search));
    $status = '&search=' . $search;
} else {
    $realsearch = '';
    $user = $controller->model->db->prepare("SELECT * FROM prices_cities ORDER BY `sort` ASC, `title` ASC ");
    $user->execute();
    $status = '';
}
