...
$controller->view->view_title = 'وزن جدید';
$controller->checker_page_access(113);
$errors = '';
if (isset($_POST['submit'])) {
    try {

        $statement = $controller->model->db->prepare("INSERT INTO `prices_weights` (`title`, `sort`) VALUES (:title, :sort)");
...
$controller->view->view_nav = "<a class='plusmenu' href='prices.weights.view.php'>بازگشت</a>" . $controller->set_fastaccess();
$controller->view->view_box_header = "<span title='قیمت گذاری -> وزن جدید'>وزن جدید {$errors}</span>";
